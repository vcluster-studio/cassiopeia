...
public class SysRoleController extends BaseController {
    ...
    public AjaxResult add(@Validated @RequestBody SysRole role) {
        ...
        // 默认将角色数据的数据权限赋予本部门
        role.setDeptIds(new Long[]{getDeptId()});
    ...
    public AjaxResult remove(@PathVariable Long[] roleIds) {
        for (Long roleId : roleIds) {
            ...
            if (sysRole == null) {
                return AjaxResult.error(MessageUtils.message("manage.role.not.exists", roleId));
            }
            if (Objects.equals(CommonEnum.SYSTEM_BUILTIN_YES.getValue(), sysRole.getIsSystem())) {
                return AjaxResult.error(MessageUtils.message("manage.role.builtin", sysRole.getRoleName()));
            }
        }
